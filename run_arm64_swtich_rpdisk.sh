qemu-system-aarch64 \
    ...
    \
    -device ioh3420,id=root_port3,chassis=1,slot=2,bus=pcie.0,addr=4\
    -drive file=./nvme30.raw,if=none,id=D30 -device nvme,bus=root_port3,drive=D30,serial=6630 \
    \
    -device ioh3420,id=root_port4,chassis=1,slot=6,bus=pcie.0,addr=6\
    -drive file=./nvme31.raw,if=none,id=D31 -device nvme,bus=root_port4,drive=D31,serial=6631 \
    \
    -device ioh3420,id=root_port6,chassis=1,slot=8,bus=pcie.0,addr=8\
    -drive file=./nvme32.raw,if=none,id=D32 -device nvme,bus=root_port6,drive=D32,serial=6632 \
    \
